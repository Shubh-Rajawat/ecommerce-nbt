...
const Footer = () => {
  return (
    <footer className="w-full absolute bottom-0   flex flex-col md:flex-row justify-center md:justify-between text-white">
      <div className="text-white px-10 py-16 lg:py-20 flex flex-col md:text-start text-center justify-center md:justify-normal  gap-5 bg-[#09405E] rounded-br-[50px]" >
        ...
        <p className=" break-words" >Lorem Ipsum is simply dummy text of the printing and typesetting industry. </p>
      ...
      <div className="flex flex-wrap justify-center md:justify-evenly md:items-center py-12 lg:py-20 w-full px-6 sm:px-8 md:px-12 lg:px-20 gap-8">
        <div className="text-4xl font-extrabold font-sans">
          Download Our App
          <p className=" break-words text-sm font-normal mt-3 text-center md:text-start" >Lorem Ipsum is simply dummy text of the printing and typesetting industry. </p>
        </div>
        <div className="flex gap-3">
          { downloadApp?.map( ( item, i ) => {
            return (
              <div className="bg-red-600 rounded-full flex items-center py-2 px-5 md:gap-3 justify-between" key={ i }>
                <div className=" lg:text-xl xl:text-4xl">{ item?.icon }</div>
                <div className="flex flex-col">
                  <span className="text-[8px]/[5px]" >{ item.text }</span>
                  <span className="text-sm md:text-md font-semibold" >{ item.name }</span>
                </div>
              </div>
            )
          } ) }
